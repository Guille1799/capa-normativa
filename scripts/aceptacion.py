...
_INV = {
    ...
    'inv-registro-md-session-start-sh': ('`python scripts/aceptaciones/censo_de_guardianes.py` — CUMPLIDA el 2026-08-24: los 45 guardianes vivos tienen ficha con su tipo y con lo que se rompe si mueren. Deja de pedir trabajo y pasa a VIGILAR que siga completa: ENUMERA de las cuatro fuentes vivas (hooks de ~/.claude/settings.json y de cada repo, repos con pre-commit, tareas del Programador cuya accion nombra un script tuyo, y los tableros que ejecuta la ronda) y se pone ROJA en cuanto aparezca uno sin describir. Instala una tarea nueva y salta sola. El umbral de 40 caracteres en el «Si muere» es lo que impide aprobar en vacio: una cabecera sola cumpliria la letra sin decir ninguna consecuencia.',
        'vigilar: el censo de guardianes sigue completo — 45 vivos, 45 descritos'),
