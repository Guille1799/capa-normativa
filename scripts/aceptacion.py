...
_INV = {
    ...
    'inv-audit-settings-source-sh-no': ('`python -c"import json,os,sys; t=json.dumps(json.load(open(r\'' + _H + '/.claude/settings.json\',encoding=\'utf-8\'))); sys.exit(1 if (\'audit_settings_source\' in t or os.path.exists(r\'' + _H + '/.claude/hooks/audit_settings_source.sh\')) else 0)"` — RETIRADO el 2026-08-24, por decision de G y con el motivo medido: el guarda preguntaba por la AUTORIA (commits de otro email en los ultimos 7 dias) y no por el CONTENIDO, asi que trabajando solo NO PODIA dispararse nunca. Este comprobador deja de pedir trabajo y pasa a vigilar que no vuelva: ROJO si el fichero reaparece o si vuelve a registrarse en settings.json. La defensa REAL no es un detector: el agujero (CVE-2025-59536) se cierra en el nivel 1 con `allowManagedHooksOnly` en managed-settings.json, que ningun repo puede pisar porque es una clave managed-only. Verificado en la doc de la v2.1.239. Esta encolado con su investigacion: mudar 18 hooks (10 de usuario + 8 de proyecto, estos con guarda para que solo actuen en su repo) es trabajo propio, no un ajuste.',
    ...
    'inv-registro-md-session-start-sh': ('`python scripts/aceptaciones/censo_de_guardianes.py` — ENUMERA los guardianes vivos de sus cuatro fuentes (hooks de ~/.claude/settings.json y de cada repo, repos con pre-commit, tareas del Programador cuya accion nombra un script tuyo, y los tableros que ejecuta la ronda) y exige por cada uno una ficha en GUARDIANES.md con Tipo y un Si-muere de 40 caracteres minimo — el umbral por debajo del cual no cabe una consecuencia, o sea la version documental de aprobar en vacio. Hoy 6 de 47. El destino cambio de REGISTRO.md a GUARDIANES.md por decision de G (2026-08-24): REGISTRO.md no es un censo sino un cuaderno de hallazgos, y 47 fichas enterrarian lo que costo medir. La lista NO se mantiene a mano: instala una tarea nueva y el censo se pone rojo solo.',
        'exprimir: el censo de guardianes — 47 vivos, y falta decir que se rompe si cada uno muere'),
...
SIN_MUTACION = {
    ...
    'inv-registro-md-session-start-sh': 'no se muta creando un fichero: enumera el estado real de la maquina (Programador de tareas, settings, repos). Su rojo SI esta cubierto por tests que lo ejercitan — tests/test_censo_de_guardianes.py comprueba que una fuente que no contesta sale ROJO en vez de contar menos guardianes, que 0 enumerados es ROJO aunque el censo este impecable, y que un guardian nuevo lo pone rojo solo.',
